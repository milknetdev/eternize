// Eternize API - bundled for Vercel
...
var LOCAL_USER = {
  id: "019f05f7-7d77-7daa-afad-3c7fb7f5bd69",
  email: "osvaldog.milk@gmail.com",
  google_user_data: {
    name: "osvaldo leite",
    given_name: "osvaldo leite",
    family_name: null,
    picture: "https://lh3.googleusercontent.com/a/ACg8ocLzNaCV-WRSlZTtdQ4uE1_QwKVwe-YnHJxMa4WfxtRIUBeZ3g=s96-c"
  }
};
...
async function authMiddleware(c, next) {
  ...
  if (!sessionToken || !sessions.has(sessionToken)) {
    ...
    sessions.set(token, { userId: LOCAL_USER.id, email: LOCAL_USER.email });
...
async function getOAuthRedirectUrl(_provider, _opts) {
  return "/auth/callback?code=local-dev-code";
}
async function exchangeCodeForSessionToken(_code, _opts) {
  const token = generateToken();
  sessions.set(token, { userId: LOCAL_USER.id, email: LOCAL_USER.email });
  return token;
}
async function deleteSession(sessionToken, _opts) {
...
// src/worker/supabase-d1.ts
var SupabaseD1Statement = class {
  client;
  sql;
  params;
...
app.use("*", async (c, next) => {
  ...
  const key = c.env?.SUPABASE_SERVICE_KEY || process.env.SUPABASE_SERVICE_KEY || "";
  if (url && key) {
    const db = new SupabaseD1(url, key);
    if (!c.env) c.env = {};
    c.env.DB = db;
    c.env.R2_BUCKET = new SupabaseR2(db.client);
  }
  await next();
});
app.get("/api/oauth/google/redirect_url", async (c) => {
  const redirectUrl = await getOAuthRedirectUrl("google", { apiUrl: "", apiKey: "" });
...
app.post("/api/sessions", async (c) => {
  ...
  const sessionToken = await exchangeCodeForSessionToken(body.code, { apiUrl: "", apiKey: "" });
...
app.get("/api/logout", async (c) => {
  ...
  if (typeof sessionToken === "string") {
    await deleteSession(sessionToken, { apiUrl: "", apiKey: "" });
  }
...
// api/index.ts
var GET = handle(worker_default);
var POST = handle(worker_default);
var PUT = handle(worker_default);
var DELETE = handle(worker_default);
var OPTIONS = handle(worker_default);
export {
  DELETE,
  GET,
  OPTIONS,
